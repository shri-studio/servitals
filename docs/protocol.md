...
```jsonc
// request
{ "secret": "<64 hex chars, generated by the agent>", "host": "nas",
  "os": "linux", "agent": "bash/1.0.0" }
// 200
{ "device_code": "<43 chars base64url>", "user_code": "WXKP-4M7R",
...
| --- | --- | --- |
| 202 | `{"status":"pending"}` | poll again after `interval` |
| 200 | `{"node_id":"…","account":"r***@gmail.com","name":"nas"}` | write credentials, start, print the account |
| 410 | `{"error":"expired"}` or `{"error":"denied"}` | stop, tell the person |
| 429 | `{"error":"slow_down"}` | add 5 s to the interval |

After a `200` the `device_code` is spent; every later request with it gets
`410`. The node's first push uses the normal signed protocol with the secret
the agent sent in `link/start`.

### 5.4 `GET /api/v1/relay/wait`
